...
def gey_100_impot_feature(train,y,nums=600):
    print('nums:',nums)
    #
    lgb_model = CatBoostRegressor()
...
def UseLightGBM():
    ...
    for train_index, test_index in kf.split(train):
        ...
        df=pd.DataFrame({'Feature':train.columns.tolist(),'importance':FeatureScore})
        df.to_csv(os.getcwd() +'/record/cbst__feautre_important_{0}.csv'.format(str(time_date)), index=False)
        sort_df=df.sort_values(by='importance',ascending=False)
        sort_df.to_csv(os.getcwd() +'/record/sort_cbst__feautre_important_{0}.csv'.format(str(time_date)), index=False)
    ...
    ####################################    提交和线下结果展示   #####################################
    res.to_csv(os.getcwd() +'/submit/cbst_%s_%s.csv'% (str(time_date), str(np.mean(xx_cv)).split('.')[1]),
